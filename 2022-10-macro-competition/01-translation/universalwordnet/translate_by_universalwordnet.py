...
# commands:
# python3 translate_by_universalwordnet.py --source_path universalwordnet.tsv --input_data ../../00-input-data/pairs-of-diminutives.tsv --output_data diminutives-cs2oth.tsv
# python3 translate_by_universalwordnet.py --source_path universalwordnet.tsv --input_data ../../00-input-data/pairs-of-social-gender.tsv --output_data social-gender-cs2oth.tsv
...
class TranslationCogNetAndUniWordNet:
    def __init__(self, source_language, target_languages, path_cognet=None, path_uniwordnet=None):
        if path_uniwordnet and source_language and target_languages:
            self.cognet = self._load_resource(path_uniwordnet, source_language, target_languages)
        if path_uniwordnet and source_language and target_languages:
...
model_cs2oth = TranslationCogNetAndUniWordNet(
    ...
    path_uniwordnet=args.source_path
)
...
with open(args.input_data, mode='r', encoding='U8') as input_file, \
    ...
    for line in input_file:
        ...
        data_line = [
            word,
            model_cs2oth.uniwordnet_translate(word=word, lang=parse_language['en']),
            model_cs2oth.uniwordnet_translate(word=word, lang=parse_language['de']),
            model_cs2oth.uniwordnet_translate(word=word, lang=parse_language['nl']),
            model_cs2oth.uniwordnet_translate(word=word, lang=parse_language['ru']),
            model_cs2oth.uniwordnet_translate(word=word, lang=parse_language['es']),
            model_cs2oth.uniwordnet_translate(word=word, lang=parse_language['fr']),
            ';'.join(list(itertools.chain(*[model_en2cs.uniwordnet_translate(word=w, lang=parse_language['cs']).split(';') for w in [item for item in model_cs2oth.uniwordnet_translate(word=word, lang=parse_language['en']).split(';')]]))),
            ';'.join(list(itertools.chain(*[model_de2cs.uniwordnet_translate(word=w, lang=parse_language['cs']).split(';') for w in [item for item in model_cs2oth.uniwordnet_translate(word=word, lang=parse_language['de']).split(';')]]))),
            ';'.join(list(itertools.chain(*[model_nl2cs.uniwordnet_translate(word=w, lang=parse_language['cs']).split(';') for w in [item for item in model_cs2oth.uniwordnet_translate(word=word, lang=parse_language['nl']).split(';')]]))),
            ';'.join(list(itertools.chain(*[model_ru2cs.uniwordnet_translate(word=w, lang=parse_language['cs']).split(';') for w in [item for item in model_cs2oth.uniwordnet_translate(word=word, lang=parse_language['ru']).split(';')]]))),
            ';'.join(list(itertools.chain(*[model_es2cs.uniwordnet_translate(word=w, lang=parse_language['cs']).split(';') for w in [item for item in model_cs2oth.uniwordnet_translate(word=word, lang=parse_language['es']).split(';')]]))),
            ';'.join(list(itertools.chain(*[model_fr2cs.uniwordnet_translate(word=w, lang=parse_language['cs']).split(';') for w in [item for item in model_cs2oth.uniwordnet_translate(word=word, lang=parse_language['fr']).split(';')]])))
        ]
